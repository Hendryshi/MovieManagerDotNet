...
namespace Utils
{
    public class FileUtility
    {
		...
		private const float MBCount = 1;
		private const float GBCount = MBCount * 1024;
		private const float TBCount = GBCount * 1024;
        ...
        public static List<string> GetPrefix(List<Movie> avs)
        {
            List<string> res = new List<string>();

            foreach (var av in avs)
            {
                var temp = av.Number.Substring(0, av.Number.IndexOf("-"));
		...
		public static float GetByteFromStr(string content)
		{
			float ret = 0f;
			...
			{
				float.TryParse(content.Replace("mb", ""), out ret);
			...
			{
				float.TryParse(content.Replace("gb", ""), out ret);
			...
			{
				float.TryParse(content.Replace("tb", ""), out ret);
